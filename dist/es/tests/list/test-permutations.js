...
describe('#permutations', () => {
    const areAllPermutesUnique = permutes => {
        const xs = permutes, limit = xs.length;
    ...
    it('Should return unique permutations for a given set of items', () => {
        const lists = 'abcd'.split('').reduceRight((agg, item, ind, list) => agg.concat([list.slice(ind)]), []); // I know laziness lol
        expectLength(4, lists);
        expectTrue(lists.every((xs, ind) => xs.length === ind + 1));
        expectTrue(lists.every(xs => {
...
//# sourceMappingURL=test-permutations.js.map