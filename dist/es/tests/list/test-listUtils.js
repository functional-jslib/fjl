...
import { sliceCopy, genericAscOrdering, lengths, toShortest, reduceUntil, reduceUntilRight, reduce, reduceRight, lastIndex, findIndexWhere, findIndexWhereRight, findIndicesWhere, findWhere } from '../../packages/list/utils';
import { expectEqual, expectFunction, expectError, alphabetString, alphabetArray, alphabetLen, vowelsString, vowelsArray, vowelsLen, vowelIndices, falsyList } from '../helpers';
describe('#listUtils', () => {
    describe('#sliceTo', () => {
    ...
    describe('#toShortest', () => {
        it('should return a list of lists trimmed to the smallest', () => {
            [
                ...
                const lists = [xs1, xs2], result = toShortest(...lists);
        ...
        it('should be curried up to 2 parameters', () => {
            [
                ...
                .forEach(([xs1, xs2, expectedLen]) => {
                const lists = [xs1, xs2], fn = toShortest(xs1), result = fn(xs2);
                expectFunction(fn);
                result.forEach((sliced, ind) => {
                    expect(sliced.length).toEqual(expectedLen);
                    expect(sliced).toEqual(lists[ind].slice(0, sliced.length));
    ...
    describe('#reduceUntil', () => {
        it('should reduce entire list when given predicate holds for all items in list', () => {
            ...
            const aggregateCharCode = (agg, item) => {
                agg.push((item + '').charCodeAt(0));
                return agg;
            }, isFalsy = x => !x, charCodes = reduceUntil(isFalsy, aggregateCharCode, [], alphabetArray);
            // Result length
        ...
        it('should reduce list up until given predicate holds for all items in list', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => agg + item, aggregand = '', result = reduceUntil(predicate, operation, aggregand, source), result2 = reduceUntil(predicate, (agg, item) => { agg.push(item); return agg; }, [], source.split(''));
        ...
        it('should be curried', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => agg + item, aggregand = '', reductionFn = reduceUntil(predicate, operation), result = reductionFn(aggregand, source), reductionFn2 = reduceUntil(predicate, (agg, item) => { agg.push(item); return agg; }), result2 = reductionFn2([], source.split(''));
    ...
    describe('#reduceUntilRight', () => {
        ...
        it('should reduce list up until given predicate holds for all items in list', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => agg + item, aggregand = '', result = reduceUntilRight(predicate, operation, aggregand, source), result2 = reduceUntilRight(predicate, (agg, item) => { agg.push(item); return agg; }, [], source.split(''));
        ...
        it('should be curried', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => agg + item, aggregand = '', reductionFn = reduceUntilRight(predicate, operation), result = reductionFn(aggregand, source), reductionFn2 = reduceUntilRight(predicate, (agg, item) => { agg.push(item); return agg; }), result2 = reductionFn2([], source.split(''));
            // Ensure funcs are functions
            [reductionFn, reductionFn2].forEach(expectFunction);
            // Test results
            expect(result).toEqual('gfed');
    ...
    describe('#reduce', () => {
        ...
        it('should be able to reduce any kind of list (string, or array, etc.)', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => predicate(item) ? agg : agg + item, aggregand = '', expectedArr = source.split('').filter(x => !predicate(x)), expectedStr = expectedArr.join(''), result = reduce(operation, aggregand, source), result2 = reduce((agg, item) => {
                if (predicate(item)) {
                    return agg;
                }
                agg.push(item);
        ...
        it('should be curried', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => predicate(item) ? agg : agg + item, aggregand = '', expectedArr = source.split('').filter(x => !predicate(x)), expectedStr = expectedArr.join(''), reductionFn1 = reduce(operation), result = reductionFn1(aggregand, source), reductioFn2 = reduce((agg, item) => {
                if (predicate(item)) {
                    return agg;
                }
                agg.push(item);
    ...
    describe('#reduceRight', () => {
        ...
        it('should be able to reduce any kind of list (string, or array, etc.)', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => predicate(item) ? agg : agg + item, aggregand = '', expectedArr = source.split('').filter(x => !predicate(x)).reverse(), expectedStr = expectedArr.join(''), result = reduceRight(operation, aggregand, source), result2 = reduceRight((agg, item) => {
                if (predicate(item)) {
                    return agg;
                }
                agg.push(item);
        ...
        it('should be curried', () => {
            const source = 'abc#!@#defg', predicate = x => !/^[a-z]$/.test(x), operation = (agg, item) => predicate(item) ? agg : agg + item, aggregand = '', expectedArr = source.split('').filter(x => !predicate(x)).reverse(), expectedStr = expectedArr.join(''), reductionFn1 = reduceRight(operation), result = reductionFn1(aggregand, source), reductioFn2 = reduceRight((agg, item) => {
                if (predicate(item)) {
                    return agg;
                }
                agg.push(item);
    ...
    describe('#findIndexWhereRight', () => {
        ...
        it('should return "found" index when predicate is matched on populated list', () => {
            expectEqual(vowelsArray.map(char => findIndexWhereRight(x => x === char, alphabetString)), vowelIndices);
        });
        it('should return `-1` when operating on empty list', () => {
            ['', []].forEach(xs => {
    ...
    describe('#findIndicesWhere', () => {
        ...
        it('should return "found" indices when predicate is matched on populated list', () => {
            const foundIndices = [].concat(...vowelsArray.map(char => findIndicesWhere(x => x === char, alphabetString)));
...
//# sourceMappingURL=test-listUtils.js.map