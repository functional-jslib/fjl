...
import { foldr } from "../../packages/list/foldr";
describe('#foldr', () => {
    it('should fold a `Foldable` (list, etc.) into some value', () => {
        ...
        expectEqual(foldr((agg, item, ind) => {
            agg += item + getAppendage(ind);
            return agg;
        }, '', vowelsString), expectedTransform.join(''));
        expectEqual(foldr((agg, item) => agg + item, 0, [1, 2, 3, 4, 5]), 15);
        expectEqual(foldr((agg, item) => agg * item, 1, [1, 2, 3, 4, 5]), 120);
        expectEqual(foldr((agg, item, ind) => {
            agg.push(item + getAppendage(ind));
            return agg;
        }, [], vowelsArray), expectedTransform);
    });
...
//# sourceMappingURL=test-foldr.js.map