...
describe('#foldl', () => {
    it('should fold a `Foldable` (list, etc.) into some value', () => {
        ...
        expectEqual(foldl((agg, item, ind) => {
            agg += item + getAppendage(ind);
            return agg;
        }, '', phrase), expectedTransform.join(''));
        expectEqual(foldl((agg, item) => agg + item, 0, [1, 2, 3, 4, 5]), 15);
        expectEqual(foldl((agg, item) => agg * item, 1, [1, 2, 3, 4, 5]), 120);
        expectEqual(foldl((agg, item, ind) => {
            agg.push(item + getAppendage(ind));
            return agg;
        }, [], phrase.split('')), expectedTransform);
    });
...
//# sourceMappingURL=test-foldl.js.map