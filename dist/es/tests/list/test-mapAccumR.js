...
describe('#mapAccumR', () => {
    it('should map a function/operation on every item of a list and it should return a tuple containing the ' +
        ...
        let xs1 = [], xs2 = '', xs3 = [];
        const list0 = [1, 2, 3, 4, 5], list1 = 'hello world', list2 = list1.split(''), stringOp = (agg, item) => String.fromCharCode(item.charCodeAt(0) + 1), numberOp = (agg, item) => item * 2, result0 = mapAccumR((agg, item) => {
            let mappedValue = numberOp(agg, item);
            agg += mappedValue;
            xs1.push(mappedValue);
            return [agg, xs1];
        }, 0, list0), result1 = mapAccumR((agg, item) => {
            let mappedValue = stringOp(agg, item);
            agg += mappedValue;
            xs2 += mappedValue;
            return [agg, xs2];
        }, '', list1), result2 = mapAccumR((agg, item) => {
            let mappedValue = stringOp(agg, item);
...
//# sourceMappingURL=test-mapAccumR.js.map