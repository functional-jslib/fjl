...
describe('#subsequences', () => {
    it('should return all sub-sequences of a sequence', () => {
        const candidates = ['abc', 'abc'.split('')], results = candidates.map(subsequences), expectedLen = Math.pow(2, candidates[0].length);
        ...
        expectTrue(results.every(result => !result.filter((subSeq, ind) => result.indexOf(subSeq) !== ind ||
            result.lastIndexOf(subSeq) !== ind).length));
...
//# sourceMappingURL=test-subsequences.js.map