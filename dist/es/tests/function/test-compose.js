...
describe('#compose', () => {
    ...
    it('should be able to compose an arbitrary number of functions and execute them as expected.', () => {
        let min = a => b => Math.min(a, b), max = a => b => Math.max(a, b), pow = a => b => Math.pow(a, b), composed = compose(min(8), max(5), pow(2)), randomNum = start => end => Math.round(Math.random() * end + start), random = randomNum(0), expectedFor = (num) => min(8)(max(5)(pow(num)(2)));
        [8, 5, 3, 2, 1, 0, random(89), random(55), random(34)].forEach(num => {
            expect(composed(num)).toEqual(expectedFor(num));
        });
    });
});
//# sourceMappingURL=test-compose.js.map