 ...
 */
import { objComplement, objDifference, objUnion, objIntersect, typeOf, instanceOf, hasOwnProperty, keys, isType, isNumber, isFunction, isArray, isBoolean, isObject, isString, isUndefined, isNull, isSymbol, isMap, isSet, isEmpty, isset, jsonClone, fromAssocList, toAssocList, toArray, log, peek, error, isWeakMap, isWeakSet, assignDeep, assign, toAssocListDeep, fromAssocListDeep, toTypeRef, toTypeRefName, lookup, native } from '../packages/object';
import { foldl, map, and, head, tail, subsequences, unfoldr, all } from '../packages/list';
import { expectTrue, expectFalse, expectEqual, expectError, expectFunction, allYourBase, alphabetString, alphabetArray, vowelsString, vowelsArray } from './helpers';
describe('#object', function () {
    const charCodeToCharArrayMap = unfoldr((charCode, ind) => ind === 26 ? undefined :
        [[charCode, String.fromCharCode(charCode)], ++charCode], 'a'.charCodeAt(0)), charCodeToCharMap = charCodeToCharArrayMap.reduce((agg, [charCode, char]) => {
        agg[charCode] = char;
        return agg;
    }, {});
    ...
    describe('#toTypeRef', () => {
        ...
        it('should return a string for all values that are not a string or a function', () => {
            [null, undefined, NaN, Symbol('abc'), Promise.resolve(), 99, [], {}].forEach(x => expect((toTypeRef(x)).constructor).toEqual(String));
        });
    });
    ...
    describe('#isType', function () {
        it('should return expected result for given values', function () {
            ...
            const truthySetWithCtors = [
                ...
                [Boolean, true, true],
                [Boolean, false, true],
            ], truthySet = truthySetWithCtors.concat(truthySetWithCtors.map(([Ctor, arg, exp]) => [Ctor.name, arg, exp])), falsySetWithCtors = [
                [Object, [], false],
                [Array, {}, false],
                ...
                [Array, false, false]
            ], falsySet = falsySetWithCtors.concat(falsySetWithCtors.map(([Ctor, arg, exp]) => [Ctor.name, arg, exp]));
            [[NaN, '', false],
                [NaN, true, false],
    ...
    describe('#assignDeep', function () {
        const sentence = 'all your base are belong to us', words = sentence.split(' '), wordsLen = words.length, obj = { all: { your: { base: { are: { belong: { to: { us: {} } } } } } } }, obj2 = { hair: { cut: { and: { shampoo: 1 } } } }, 
        ...
        randomObj = words.reduce((agg, word, ind) => {
            ...
            return agg;
        }, {}), clonedObj = jsonClone(obj), originalObjects = {
            obj2: jsonClone(obj2),
        ...
        it('should assign all props from one object to another recursively', () => {
            ...
            const check1 = foldl(([_results, _obj, _lastObj], word) => [
    ...
    describe('#objComplement', function () {
        ...
        it('should return an object with only properties not found in the first obj', function () {
            let subj1 = { a: 1, b: 2, c: 3 }, subj2 = { d: 4 }, subj3 = { e: 5, f: 6, g: 7 }, result = objComplement(subj1, subj2, subj3);
    ...
    describe('#objDifference', function () {
        ...
        it('should return all the props from obj1 that aren\'t in obj2', function () {
            let subj1 = { a: 1, b: 2, c: 3 }, subj2 = { d: 4 }, result = objDifference(subj1, subj2);
    ...
    describe('#objUnion', function () {
        ...
        it('should return an object containing all properties from the two objects passed in', function () {
            let subj1 = { a: 1, b: 2, c: 3 }, subj2 = { e: 5, f: 6, g: 7 }, result = objUnion(subj1, subj2);
    ...
    describe('#objIntersect', function () {
        ...
        it('should return an object that contains values from both passed in objects', function () {
            let subj1 = { a: 1, b: 2, c: 3, e: 4, f: 8 }, subj2 = { a: 5, b: 6, c: 7, g: 9 }, sharedKeys = ['a', 'b', 'c'], result = objIntersect(subj1, subj2);
    ...
    describe('#toArray', function () {
        ...
        test('should return an array for values that can be converted to an array', () => {
            [
                ...
                .forEach(([given, expected]) => expect(toArray(given)).toEqual(expected));
        });
    });
    ...
    describe('#toAssocListDeep', () => {
        it('should be able to turn an object into an associated list', () => {
            const expected = [[
                ...
                ]], result = toAssocListDeep(allYourBase, allYourBase.constructor), verifyResult = (assocList, obj) => {
    ...
    describe('#fromAssocList, #fromAssocList', function () {
        test('should return an object from an array map', () => {
            ...
            expect(all(([charCode, char]) => result[charCode] === char, charCodeToCharArrayMap))
                .toEqual(true);
        });
    ...
    describe('#fromAssocListDeep', () => {
        ...
        it('should throw an error when receiving anything other than an array or reducible', () => {
            ...
            [null, undefined, 99, true, Symbol('99'), 'hello'].forEach(x => expectError(() => fromAssocListDeep(x)));
        });
    });
    describe('#lookup', function () {
        it('should return found value when key is set on type instance', function () {
            const word = 'hello world', obj = word.split('').reduce((agg, item) => {
                agg[item] = item + ' value';
                return agg;
            }, {});
            expectTrue(all(() => all(elm2 => lookup(elm2, obj) === elm2 + ' value', word), [word.split(''), word]));
        });
        it('should return `undefined` when element is not found in given list', function () {
            ...
            expectTrue(all(elm => all(() => lookup('z', obj) === undefined, elm), [word.split(''), word]));
        });
    });
    describe('#native', () => {
        describe('#functions-on-native', () => {
            Object.getOwnPropertyNames(Object).forEach(key => {
                const foundOnNative = native[key], foundOnObj = Object[key];
                // Test functions only
