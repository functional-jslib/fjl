...
import { createTypedDescriptor, toEnumerableDescriptor, defineProp, defineProps, defineEnumProp, defineEnumProps } from '../../packages/object/defineProp';
describe('#object.createTypedDescriptor', function () {
    const someTarget = {}, exampleNumberDescriptor = createTypedDescriptor(Number, someTarget, 'someNum'), result = exampleNumberDescriptor;
...
describe('#object.defineProp', function () {
    const someTarget = {}, propName = 'someNum', [target, descriptor] = defineProp(Number, [someTarget], propName);
    ...
    it('should allow the user to pass in his/her own `descriptor`', function () {
        const somePropName = 'somePropName', someValue = (new Date()).getTime(), customDescriptor = {
            value: someValue,
            enumerable: true
...
describe('#object.defineEnumProp', function () {
    const someTarget = {}, propName = 'someNum', [target, descriptor] = defineEnumProp(Number, [someTarget], propName);
    it('should return a `target` and `descriptor` pair (tuple)', function () {
        expect(target).toEqual(someTarget);
        expect(!!descriptor).toEqual(true);
    });
...
describe('#object.defineProps', function () {
    const seedArgTuples = [
        ...
        agg[tuple[1] + ''] = null;
        return agg;
    }, {}), seedPropNames = keys(seedTarget), generateTargetData = () => unfoldr((argTuples, ind, _out) => {
        const _argTuples = argTuples.slice(0), out = [_argTuples.slice(0), {}];
        if (!_out.length) {
    ...
    it('should be able to define many props on given target with only argTuples of length `2`', function () {
        generateTargetData().forEach(args => {
            // log(args);
            const target = defineProps.apply(null, args), propNames = args[0].map(x => x[1]);
            // log(propNames, '\n', target);
    ...
    it('should return target with defined properties from operation', function () {
        generateTargetData().forEach(args => {
            // log(args);
            const target = apply(defineProps, args), argKeyNames = args[0].map(pair => pair[1]);
    ...
    it('should be able to set types with argTuples of length of `3` (containing a `defaultValue`)', function () {
        generateTargetData().map(argTuple => {
            ...
            return [
                ...
                args.map((argSet, ind) => {
                    const [TypeRef, propName] = argSet, [correct] = seedArgTupleCorrectIncorrectValues[ind];
            ...
            // log(args);
            const target = apply(defineProps, args), argKeyNames = args[0].map(([_, key]) => key);
            expect(target).toBeInstanceOf(Object);
            argKeyNames.forEach(key => {
                const propDescriptor = Object.getOwnPropertyDescriptor(target, key);
                expect(['set', 'get'].every(k => propDescriptor[k] instanceof Function));
...
describe('#object.defineEnumProps', function () {
    ...
    it('should be able to define many enum props on given target with only argTuples of length `2`', function () {
        generateTargetData().forEach(args => {
            // log(args);
            const target = apply(defineEnumProps, args), propNames = args[0].map(([_, name]) => name);
            // log(propNames, '\n', target);
...
//# sourceMappingURL=test-object.defineProp.js.map