...
const _permutationsAlgo = (list, remainderLen, listLen) => {
        const out = [list];

        if (remainderLen === 1) { return list; }

        for (let i = 0; i < remainderLen; i++) {
            const newLen = remainderLen - 1;
            out.push(
                _permutationsAlgo(list, newLen, listLen)
            );

            // If remainderLen is odd, swap first and last element
            //  else, swap ith and last element
            out.push(
                _swap(list, (remainderLen % 2 === 1 ? 0 : i), newLen)
            );
        }
        return out;
    };
